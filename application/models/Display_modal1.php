...
//          $this->email->send ();
  ...
  //       return $query;   
	...
	{
		// $selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.product_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 0 AND dc_cancel_order = 0 AND order_status = 0 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		// $query = $this->db->query($selquery);
	...
	{
		// $selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.product_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 1 AND order_status = 1 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		// $query = $this->db->query($selquery);  
	...
	{
		// $selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.product_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 0 AND order_status = 0 AND dc_cancel_order = 0 AND MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE()) GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		// $query = $this->db->query($selquery); 
