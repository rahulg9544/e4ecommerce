...
//          $this->email->send ();
	...
	function getcommcalforadmin2($filt,$joincondition)
	{
		...
		$selquery = "SELECT * FROM `deliverycharge` LEFT JOIN user ON deliverycharge.dc_agent_id = user.user_id LEFT JOIN store ON user.user_agent_id = store.store_id LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id
  ...
  //       return $query;   
	...
	function admincalcommission()
	{
		...
		$selquery = "SELECT SUM(prod_addedcomm) - SUM(prod_rate) as newvalue FROM `product` WHERE prod_admin_approved = 1"; 
		$query = $this->db->query($selquery); 
	...
	// function adminmonthlycommission(){
	// 	$selquery = "SELECT SUM(prod_addedcomm) - SUM(prod_rate) as newvalue FROM `product` WHERE prod_admin_approved = 1"; 
	// 	$query = $this->db->query($selquery); 
 //        return $query->row()->newvalue; 
	// }
	// function agentmonthlycommission(){
	// 	$selquery = "SELECT SUM(prod_addedcomm) - SUM(prod_rate) as newvalue FROM `product` WHERE prod_admin_approved = 1 AND prod_agent_id = '$agentid' AND MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE())"; 
	// 	$query = $this->db->query($selquery); 
 //        return $query->row()->newvalue;
	// 	 WHERE   
	// }
	function countbanslide($a)
	{
		$this->db->where('banner_slider_status',0);
		$query = $this->db->get($a);  
        return $query->num_rows();   
	}
	...
	{
		$selquery = "SELECT  IFNULL(sum(deliverycharge.dc_prod_actualstoreprice),0) as subtotal FROM `deliverycharge` WHERE deliverycharge.order_status = 1 AND deliverycharge.dc_status = 1 AND deliverycharge.dc_cancel_order = 0";
		$query = $this->db->query($selquery);  
        return $query->row()->subtotal;   
	}
	function sumofmonthlysalesforadmin()
	{
		$selquery = "SELECT  IFNULL(sum(deliverycharge.dc_prod_actualstoreprice),0) as subtotal FROM `deliverycharge` WHERE deliverycharge.order_status = 1 AND deliverycharge.dc_status = 1  AND deliverycharge.dc_cancel_order = 0 AND MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE())";
		$query = $this->db->query($selquery);  
        return $query->row()->subtotal;   
	}
	...
	{
		$this->db->where('prod_admin_approved', 1);
		$query = $this->db->get('product');  
        return $query->num_rows();   
	}
	...
	function countusers()
	{
		...
		$selquery = "SELECT COUNT(IF(user_type='user',1, NULL)) 'user', COUNT(IF(user_type='agent',1, NULL)) 'agent' FROM user";
	...
	{
		$selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id  WHERE deliverycharge.dc_agent_id = '$userid' AND dc_status = 0 AND order_status = 0 AND dc_cancel_order = 0 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC ";
	...
	{
		// $selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 0 AND dc_cancel_order = 0 AND order_status = 0 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		// $query = $this->db->query($selquery);
	...
	{
		$selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id  WHERE deliverycharge.dc_agent_id = '$userid' AND dc_status = 1 AND order_status = 1 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC ";
	...
	{
		// $selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 1 AND order_status = 1 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		// $query = $this->db->query($selquery);  
        $this->db->where('orders_status',2);
        $query = $this->db->get('orders');
	...
	{
		$selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 1 AND order_status = 1 AND  MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE()) GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		$query = $this->db->query($selquery);  
        return $query->num_rows();   
	}
	function countmonthlydeliveredbyagent($userid)
	{
		$selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id  WHERE deliverycharge.dc_agent_id = '$userid' AND dc_status = 1 AND order_status = 1 AND  MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE()) GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC ";
	...
	{
		// $selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE dc_status = 0 AND order_status = 0 AND dc_cancel_order = 0 AND MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE()) GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		// $query = $this->db->query($selquery); 
	...
	{
		$selquery = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id  WHERE deliverycharge.dc_agent_id = '$userid' AND dc_status = 0 AND dc_cancel_order = 0 AND order_status = 0 AND  MONTH(dc_date) = MONTH(CURRENT_DATE()) AND YEAR(dc_date) = YEAR(CURRENT_DATE()) GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC ";
	...
	function displayproduct($a)
	{
		...
		// $query = $this->db->get($a);  
         ...
         $squery="SELECT product.prod_id,product.prod_rate,product.prod_unique_id,product.prod_sell_price,product.prod_brand_id,product.prod_image,product.prod_deactive,product.prod_packtype,product.prod_rating,product.prod_stock_qty,product.prod_short_desc,product.prod_disc,product.prod_descr,review.review_prod_id,count(review.review_prod_id) as reviewcont,subcategory.sub_id,product.prod_name,product.prod_code,product.prod_uom,brand.brand_name,user.user_id,user.user_businessnameame,category.cat_name,category.cat_id from review left join product on product.prod_id = review.review_prod_id LEFT join subcategory on subcategory.sub_id=product.prod_sub_cat_id LEFT join brand on brand.brand_id=product.prod_brand_id LEFT JOIN user on product.prod_agent_id=user.user_id LEFT JOIN category ON category.cat_id=product.prod_cat_id WHERE product.prod_admin_approved = 1 GROUP BY review.review_prod_id ORDER BY product.prod_rating DESC LIMIT 10";
	...
	function displaybanner($a)
	{
		...
		$selquery = "SELECT * FROM `$a` LEFT JOIN category ON banner_slider.banner_slider_category = category.cat_id LEFT JOIN subcategory ON banner_slider.banner_slider_subcategory = subcategory.sub_id LEFT JOIN brand ON banner_slider.banner_slider_brand = brand.brand_id WHERE banner_slider.banner_slider_status = 1 AND banner_slider.banner_slider_choose = 0 GROUP BY banner_slider.banner_slider_id";
		$query = $this->db->query($selquery);  
        return $query->result();   
	}
	...
	function display_recent_product()
	{
		...
		$selquery = "SELECT * FROM product LEFT JOIN category ON product.prod_cat_id = category.cat_id WHERE product.prod_admin_approved = 1  ORDER BY product.prod_id DESC LIMIT 3";
		$query = $this->db->query($selquery);  
        return $query->result();   
	}
	...
	function displayslider($a)
	{
		$selquery = "SELECT * FROM `$a` LEFT JOIN category ON banner_slider.banner_slider_category = category.cat_id LEFT JOIN subcategory ON banner_slider.banner_slider_subcategory = subcategory.sub_id LEFT JOIN brand ON banner_slider.banner_slider_brand = brand.brand_id WHERE banner_slider.banner_slider_status = 1 AND banner_slider.banner_slider_choose = 1 GROUP BY banner_slider.banner_slider_id";
		$query = $this->db->query($selquery);  
        return $query->result();     
	}

	// function search_product($term){
	// 	$selqry = "SELECT prod_name FROM product WHERE prod_admin_approved = 1 AND (prod_name LIKE '%".$term."%' OR prod_descr LIKE '%".$term."%') ORDER BY prod_name ASC LIMIT 13";
	// 	$query = $this->db->query($selqry);
	// 	return $query->result();
	// }
	function search_product($searchitem,$stores)
	{
	...
	function displayproductbyloc($a,$stores,$mode)
	{	
		// if($mode == 'tab-weekly'){
		...
		// 	$this->db->where('prod_modified >=', $week_start);
		// 	$this->db->where('prod_modified <=', $week_end);
		...
		if($mode == 'tab-latest'){
			$this->db->order_by('prod_modified','DESC');
		}else{
			$this->db->order_by('prod_modified', 'RANDOM');
		}
	...
	{
		$selquery = "SELECT * FROM product LEFT JOIN store ON store.store_id = product.prod_store_id WHERE product.prod_id = '$id'";
		$query = $this->db->query($selquery);
		// $this->db->where('prod_id',$id);  
		// $query = $this->db->get($a);  
        return $query->row();   
	...
	{
			$selquery = "SELECT * FROM `product` LEFT JOIN category ON product.prod_cat_id = category.cat_id LEFT JOIN subcategory ON product.prod_sub_cat_id = subcategory.sub_id LEFT JOIN brand ON product.prod_brand_id = brand.brand_id WHERE product.prod_id = '$id'"; 
	...
	{
		$selquery = "SELECT * FROM `product` LEFT JOIN category ON product.prod_cat_id = category.cat_id LEFT JOIN subcategory ON product.prod_sub_cat_id = subcategory.sub_id WHERE product.prod_id = '$id'"; 
		$query = $this->db->query($selquery);  
        $excute =  $query->row();  
	...
	public function getcat_brandsbycatid($category_id,$stores)
	{
		...
		$this->db->where('product.prod_admin_approved', 1);
		...
		// $selqry = "SELECT * FROM `product` LEFT JOIN brand ON product.prod_brand_id = brand.brand_id WHERE product.prod_cat_id = '$category_id' AND prod_admin_approved = 1 and prod_brand_id != 0  GROUP BY brand.brand_id";
		//  $query = $this->db->query($selqry);
	   $query = $this->db->get();
	    return $query->result();
	}
	public function getcat_brandsbybrandid($brand_id,$stores)
	{
		...
		// $selqry = "SELECT * FROM `product` LEFT JOIN brand ON product.prod_brand_id = brand.brand_id WHERE product.prod_brand_id = '$brand_id' AND prod_admin_approved = 1 and prod_brand_id != 0  GROUP BY brand.brand_id";
	    $query = $this->db->get();
	    return $query->result();
	}
	public function getcat_brandsbysubcatid($category_sub_id,$stores)
	{
		...
		$this->db->group_by('brand.brand_id');
		// $selqry = "SELECT * FROM `product` LEFT JOIN brand ON product.prod_brand_id = brand.brand_id WHERE product.prod_sub_cat_id = '$category_sub_id' AND prod_admin_approved = 1 and prod_brand_id != 0 GROUP BY brand.brand_id";
	    $query = $this->db->get();
	    return $query->result();
	}
	...
	public function allitems($ordid){
		$selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN brand  ON product.prod_brand_id = brand.brand_id WHERE deliverycharge.dc_order_id = '$ordid'";
		$query = $this->db->query($selqry);
		return $query->result();
	}
	...
	function getautoproductsbycategory($stores){
		...
		// $selquery = "SELECT * FROM `product` LEFT JOIN category ON product.prod_cat_id = category.cat_id WHERE product.prod_admin_approved = 1 AND product.prod_priority = 0 GROUP BY product.prod_id ";  
		// $query = $this->db->query($selquery);  
        return $query;   
	}
	...
	function countcartold($userid){
		$selqry = "SELECT count(*) as cartcount FROM `cart` LEFT JOIN product ON cart.cart_product_id = product.prod_id WHERE cart_user_id = '$userid' AND  prod_deactive = 0 AND cart_ordr_status = 0";
		$query = $this->db->query($selqry);  
        return $query->row();  
	}
	function countcart($userid,$stores){
		$this->db->select('*');
		$this->db->from('cart');
		$this->db->join('product', 'cart.cart_product_id = product.prod_id', 'left');
		$this->db->where('cart_user_id', $userid);
		...
		$query = $this->db->get();
		// $selqry = "SELECT * FROM `cart` LEFT JOIN product ON cart.cart_product_id = product.prod_id  WHERE cart_user_id = '$userid' AND cart_ordr_status = 0 AND product.prod_deactive = 0 GROUP BY cart.cart_id";
		// $query = $this->db->query($selqry);  
        return $query->num_rows();  
	}
	...
	function cartitems($userid){
		$selqry = "SELECT * FROM `cart` LEFT JOIN product ON cart.cart_product_id = product.prod_id  WHERE cart_user_id = '$userid' AND cart_ordr_status = 0
		AND prod_deactive = 0 GROUP BY cart.cart_id";
		$query = $this->db->query($selqry);  
        return $query->result();  
	}
	function getstoreidincart($userid,$stid){
		$selqry = "SELECT * FROM cart LEFT JOIN product ON cart.cart_product_id = product.prod_id WHERE cart_user_id = '$userid' AND product.prod_store_id = '$stid' AND cart.cart_ordr_status = 0 AND product.prod_deactive = 0";
		$query = $this->db->query($selqry);
		 return $query->result();
	}
	...
	function wishlistitems($userid){
		$selqry = "SELECT * FROM `wishlists` LEFT JOIN product ON wishlists.wishlists_prod_id = product.prod_id  WHERE wishlists.wishlists_user_id = '$userid' GROUP BY wishlists.wishlists_id";
		$query = $this->db->query($selqry);  
        return $query->result();  
	}
 ...
 //    function display_selected_id($a)
  ...
  function getsubcats()
  {
  	...
  	$squery = "SELECT product.*,subcategory.*, dc_prod_id, COUNT(*) AS counts FROM deliverycharge LEFT JOIN product ON deliverycharge.dc_prod_id = product.prod_id LEFT JOIN  subcategory ON subcategory.sub_id = product.prod_sub_cat_id GROUP BY dc_prod_id HAVING COUNT(*) > 0 ORDER BY counts DESC LIMIT 10";
  ...
  function getnewprods()
  {
     ...
     // $selqry="SELECT * FROM product WHERE prod_admin_approved= 1 AND prod_priority= 0 AND prod_modified > '$sevenday' LIMIT 20";

    $squery="SELECT product.prod_id,product.prod_rate,product.prod_unique_id,product.prod_sell_price,product.prod_brand_id,product.prod_image,product.prod_deactive,product.prod_packtype,product.prod_rating,product.prod_stock_qty,product.prod_short_desc,product.prod_disc,product.prod_descr,review.review_prod_id,count(review.review_prod_id) as reviewcont,subcategory.sub_id,product.prod_name,product.prod_code,product.prod_uom,brand.brand_name,user.user_id,user.user_businessnameame,category.cat_name,category.cat_id from review left join product on product.prod_id = review.review_prod_id LEFT join subcategory on subcategory.sub_id=product.prod_sub_cat_id LEFT join brand on brand.brand_id=product.prod_brand_id LEFT JOIN user on product.prod_agent_id=user.user_id LEFT JOIN category ON category.cat_id=product.prod_cat_id WHERE product.prod_admin_approved = 1 AND product.prod_priority= 0 AND product.prod_modified > '$sevenday' GROUP BY review.review_prod_id ORDER BY product.prod_modified DESC LIMIT 10";
  ...
  function getnewarrprods()
  {
     ...
     $selqry="SELECT * FROM product WHERE prod_admin_approved= 1 AND prod_priority= 0 AND prod_modified > '$sevenday'";
     $query= $this->db->query($selqry);
     return $query->result();
  }
  ...
  function getlargnewarrv()
  {
     ...
     $selqry="SELECT * FROM product WHERE prod_admin_approved= 1 AND prod_priority= 0 AND prod_modified > '$sevenday' ORDER BY prod_sell_price DESC LIMIT 1";
  ...
  function getsmalnewarrv()
  {
     ...
     $selqry="SELECT * FROM product WHERE prod_admin_approved= 1 AND prod_priority= 0 AND prod_modified > '$sevenday' ORDER BY prod_sell_price ASC LIMIT 1";
     $query1= $this->db->query($selqry);
     if($query1->num_rows()!=0)
     {
     $query=$query1->row();
  ...
  {
  	$selqry="SELECT * FROM product WHERE prod_admin_approved= 1 AND prod_priority= 0 AND prod_brand_id = '$brandid' ";
     $query= $this->db->query($selqry);
     return $query->result();
  }
  ...
  function getslidesall()
  {
  	$this->db->where('banner_slider_status','1');
  	$this->db->order_by('banner_slider_showprio','ASC');
    $query = $this->db->get('banner_slider');
    return $query->result();
  ...
  {
  	$this->db->where('banner_slider_status','1');
  	$this->db->where('banner_slider_showprio','6');
  	$query = $this->db->get('banner_slider');
  	return $query->row();
  }
  ...
  {
  	$this->db->where('banner_slider_status','1');
  	$this->db->where('banner_slider_showprio','7');
  	$query = $this->db->get('banner_slider');
  	return $query->row();
  }
