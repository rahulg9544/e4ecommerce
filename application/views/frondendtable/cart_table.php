...
<?php } ?> 
    ...
    <td class="cart_description coinzer">
        <p class="s_title_block">
            ...
            <?php if($row->cart_color=='n/a' || $row->cart_color=='') {  

                if($row->cart_sizecolorstat==3) {
                    ?>
                   <a span="">
                <span>Color : <?php echo $row->cart_color; ?></span><br><span>Size : <?php echo $row->cart_size ?></span>
                    </a> 
                    <?php 
                }
            }
            ...
            {    
            if(strpos($row->cart_color, '.') !== false){ ?>	
             <a span="">
...
{ ?>
   ...
   <td class="cart_unit">
                <span class="price" id="product_price_5806_17409_0">
                <span> KWD <?php echo number_format($row->product_sell_price,3); ?></span>
                
                <?php if($row->product_discount!=0 && $row->product_discount!='') { ?>

                <span class="without-reduction" style="text-decoration:line-through; color:#ccc;"> KWD <?php echo number_format($row->product_rate,3); ?> </span>
                </span>
...
<input type="hidden" name="carttotlinp" id="carttotlinp" value="<?php  $carttotlinp = number_format($carttotal,3);  
echo str_replace(',', '', $carttotlinp); ?>">

<input type="hidden" name="cartgshipcinp" id="cartgshipcinp" value="<?php echo number_format(0,3); ?>"   >

<?php $cgtotal =$carttotal+0 ?>

<input type="hidden" name="cartgtotlinp" id="cartgtotlinp"  value="<?php  $cgtotal = number_format($carttotal,3);  
echo str_replace(',', '', $cgtotal); ?>">
