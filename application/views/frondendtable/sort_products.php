...
<?php } ?>                
        ...
        <ul>
          ...
          <?php if(isset($_SESSION['cusername'])) { 
            ...
            <li class="wishlist"><a href="<?php echo base_url(); ?>Login" title="Add to Wishlist"><i class="icon-heart icons"></i></a></li>
...
<a class="icon-heart add" href="<?php echo base_url(); ?>Login" title="Add to Wishlist">
</a>
...
<?php } p?>
                            ...
                            <div class="clear"></div>
<?php 
if($row->product_size_status==1 || $row->product_size_status==3) 
{ ?>
<!-- size -->                        
      ...
      for($p=0;$p<count($exp_size);$p++)
      {
        $psize=$exp_size[$p];
          if($jsize==$psize)
          {
            ...
            if($row->product_size_status==3)
            {
            $sizeviseclr .= "empty"."=".$exp_price[$p].",";
            }
            else
            {
...
<div>
   <input type="radio" id="sizeno<?php echo $q ?>-0" name="sizeno<?php echo $q ?>" value="<?php echo $size ?>" checked>
    <label for="sizeno<?php echo $q ?>-0">
      <span><?php echo $size ?>
      </span>
    </label>
</div>
...
if($row->product_size_status==1 || $row->product_size_status==2) 
{
  ?>
<!-- color -->
...
?>
 <h6>Select Color</h6>
 <?php if( strpos( $size,',') !== false && $row->product_size_status!=2)
     { ?>
...
<?php }
//start of colour only
else if($row->product_size_status==2)
  {
    ?>
    <div id="colorfill<?php echo $q; ?>">
    <div>
    <?php
    $color_array = explode(",",$row->product_color);
    $price_array = explode(",",$row->product_sellpricewise);
    for($i=0;$i<count($color_array);$i++)
    {
      ?>
       <input type="radio" id="color<?php echo $q ?>-<?php echo $i; ?>" name="color<?php echo $q ?>" value="<?php echo $color_array[$i]; ?>"  onclick="showitspriz('<?php echo $price_array[$i] ?>','<?php echo $q ?>','<?php echo $color_array[$i] ?>','1')">
    
    <?php if(strpos($color_array[$i],'.') !== false) { ?>

    <label for="color<?php echo $q ?>-<?php echo $i; ?>" id="colorlab<?php echo $q ?>-<?php echo $i; ?>" style="border:0.5px solid;background-image: url(<?php echo base_url(); ?>uploads/<?php echo $color_array[$i]; ?>);"><span id="colorspn<?php echo $q ?>-<?php echo $i; ?>"></span></label>

  <?php } else { ?>

     <label for="color<?php echo $q ?>-<?php echo $i; ?>" id="colorlab<?php echo $q ?>-<?php echo $i; ?>" style="border:0.5px solid;background-color: <?php echo $color_array[$i]; ?>"><span id="colorspn<?php echo $q ?>-<?php echo $i; ?>"></span></label>
...
else
{
  ...
  <div>
    ...
    <input type="radio" id="color<?php echo $q ?>-0" name="color<?php echo $q ?>" value="<?php echo $row->product_color ?>" checked>
    
    <?php if(strpos( $row->product_color,'.') !== false) { ?>

    <label for="color<?php echo $q ?>-0" id="colorlab<?php echo $q ?>-0" style="border:0.5px solid;background-image: url(<?php echo base_url(); ?>uploads/<?php echo $row->product_color ?>);"><span id="colorspn<?php echo $q ?>-0"></span></label>

  <?php } else { ?>

     <label for="color<?php echo $q ?>-0" id="colorlab<?php echo $q ?>-0" style="border:0.5px solid;background-color: <?php echo $row->product_color ?>"><span id="colorspn<?php echo $q ?>-0"></span></label>
...
<?php } else { ?>
   ...
   <input type="hidden" value="<?php echo $row->product_color ?>" id="selecolor<?php echo $q ?>" name="selecolor<?php echo $q ?>" >

 <input type="hidden" value="<?php echo $size ?>" id="selesize<?php echo $q ?>" name="selesize<?php echo $q ?>">
...
<?php if(isset($_SESSION['cusername']) || isset($_SESSION['cgustid'])) { 
      ...
      <!-- <form action="#"> -->
          ...
          <button onclick="additemcart('<?php echo $row->product_id ?>','<?php echo $q ?>','<?php echo $row->product_size_status ?>')">add to cart</button>
      <!-- </form> -->
  <?php } ?>    
  </div>   
                                    </div>
